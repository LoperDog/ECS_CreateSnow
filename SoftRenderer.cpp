...
int g_nClientWidth = 512;
int g_nClientHeight = 512;
int g_elapsedTime = 0;
int g_minTime = 1000;
int g_maxTime = 0;
bool g_bIsActive;
...
int MsgLoop(HACCEL hAccelTable)
{
	...
	while (true)
	{
      ...
      world->tick();
      //sw.Stop();
      deltatime = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - frametime);

			//g_elapsedTime = sw.ElapsedMilliseconds();
      g_elapsedTime = deltatime.count();
		}
		else
		{
			WaitMessage();
...
inline void SetWorld() {
  world = std::make_shared<JT::World>();
  std::shared_ptr<RenderSystem> rend = std::make_shared<RenderSystem>(world);
  std::shared_ptr<AddSystem> addsys = std::make_shared<AddSystem>(world);

  world->AddSystem(rend);
  world->AddSystem(addsys);
}
